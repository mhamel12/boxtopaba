...
#                            print "Checking: $link_month_day\n";
                            if (exists $game_days{$link_month_day})
                            {
                                ...
                                print output_filehandle "<td style=\"text-align:center\"><a href=#$link_month_day>$months{$this_column}{days_generated}</a></td>";
                            }
                            else
                            {        
...
#    print "column count = $column_count\n";
    ...
    if ($info{radio} ne "") { print output_filehandle "Boston Radio: <span style=\"color:gray\">$info{radio}</span><br>\n"; }
    if ($info{tv} ne "") { print output_filehandle "Boston TV: <span style=\"color:gray\">$info{tv}</span>\n"; }
...
#            $link_text = sprintf("Game_%02d_%02d_%s_%s",$mon,$mday,$home_abbrev,$road_abbrev);
            ...
            $link_text = Boxtop_GetGameLinkText($index_date{$index_scan},$index_hteam{$index_scan},$index_rteam{$index_scan},$index_gamenumber{$index_scan});
